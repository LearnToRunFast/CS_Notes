...
For R-way Trie, there are too many null slots. With ternary tire, there are three branches:

1. `Left`:The nodes inside left are strictly smaller than current char.
2. `mid`: The nodes inside mid are equal than current char.
3. `right`: The nodes inside right are strictly bigger than current char.
